...
var ErrInvalidRuntimeExpression = errors.New("disruptor: invalid runtime expression")

// RuntimeVariables exposes read-only runtime variable lookup.
type RuntimeVariables = runtimevars.Variables

// RuntimeExpression is a compiled-at-registration runtime graph expression.
type RuntimeExpression string

// Expression is the package-local name for RuntimeExpression.
type Expression = RuntimeExpression
...
// Compiler compiles expressions into bool evaluators.
type Compiler = ExpressionCompiler
...
type Request = ExpressionRequest
...
type ValueKind = ExpressionValueKind
...
type ValueConvertRequest = ExpressionValueConvertRequest
...
type ValueConverter = ExpressionValueConverter
...
type ValueConverterFunc = ExpressionValueConverterFunc
...
// CompilerOption configures the default expression compiler.
type CompilerOption = RuntimeExpressionCompilerOption
...
func WithValueConverter(converter ValueConverter) CompilerOption {
	return WithExpressionValueConverter(converter)
}
...
func NewCompiler(opts ...CompilerOption) Compiler {
	return NewRuntimeExpressionCompiler(opts...)
}
