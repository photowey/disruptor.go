...
- `ringbuffer.RingBuffer[T]` 是底层 API，用于申请、修改和发布预分配事件槽。
- `disruptor.Disruptor[T]` 是高层门面，围绕一个 Ring Buffer 管理 processor。
...
- `event.Factory[T]`、`event.Translator[T]`、`event.Handler[T]`、
  `event.ExceptionHandler[T]`、`wait.Strategy` 和 `metrics.Sink` 都是接口。
...
variables，表达式边再读取这些变量。`event.Request.Runtime` 会在事件之间
被重置，仅在当前 handler 回调内有效。
...
`${flags} & 1` 这类整数位运算。
默认事件 resolver 和自定义 runtime variable source 也可以实现
`runtimevars.TypedResolver[T]` 或 `runtimevars.TypedVariables`，让标量值在进入
表达式引擎前保持 typed 路径，减少装箱。
默认数值比较保持轻量：有符号和无符号整数使用精确比较，float 操作数使用 Go
`float64` 语义。decimal、money、big number 等领域数值通过
`expression.WithNumberAdapter(adapter)` 扩展。NumberAdapter 可以把普通变量
或 typed object 变量转换为 `ValueNumber`，负责自定义数值比较，并且只在表达式
最终结果为自定义数值时执行 bool 转换。decimal 语义归 adapter 所有，不进入核心
模块依赖图。
...
flowchart LR
    ...
    Capacity -->|否| Wait["等待或 ringbuffer.ErrInsufficientCapacity"]
    Slot --> Pub["Publish"]
    Pub --> B["Barrier"]
...
公共包按职责拆分。`pkg/disruptor` 负责高层编排；其他包负责底层基础设施、
可替换契约、拓扑构建和 runtime graph 所需能力：

```text
pkg/disruptor/    fan-out、静态 graph 和 runtime graph 的门面编排
pkg/event/        handler request、生命周期 hook、exception handler
pkg/sequence/     公共 sequence 类型和 reader
pkg/wait/         wait strategy 接口以及 blocking/busy-spin 内置实现
pkg/metrics/      后端无关的 metrics sink 和 payload
pkg/ringbuffer/   预分配事件存储、barrier、producer option
pkg/processor/    event processor 生命周期和 processor option
...
`pkg/sequence.Sequence` 是稳定的公共 sequence 类型。single-producer 和
multi-producer sequencing 具体算法仍然保留在 `internal` 内。
...
默认异常处理器是 fail-fast。可替换为 ignore 或有界 retry：
...
普通所有权转移和简单同步仍然优先使用 channel。只有当 benchmark 证明应用需要高吞吐、低分配、广播给多个消费者或可控背压时，再使用这个库。
